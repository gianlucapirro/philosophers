...
/*   Updated: 2022/07/04 13:32:40 by gpirro        ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
WRONG_AMOUNT_OF_ARGUMENTS));
	...
	while (i < argc)
	{
		if (is_valid_input(argv[i]) == false)
			return (p_error("Error: input is negative, not a long int or\
not an int", INT_OVERFLOW));
		i++;
	}
	return (0);
}
